...
void initialConfigPacket()
{
	...
	char hello[48];
    // Send the data
	int no_of_packets = (data_value.F_signal * data_value.t_acq);	
...
//	char *getConfBuf1 = "Device_ID:IRD01;channels:4;samples:2200";
	...
	snprintf(getConfBuf1, sizeof(getConfBuf1),
                       "Device_ID:IRD01;channels:4;No of samples:%d",
                      no_of_packets );
	printf("config-packet: %s\n\r",getConfBuf1);
	tcp_sent(pcb, client_sent);
	//fsm_add_event(EV_CONFIG_PACKET);
	tcp_write(pcb, getConfBuf1, strlen(getConfBuf1), TCP_WRITE_FLAG_COPY);
	tcp_output(pcb);
}

void send_lastpacket()
{
	char *lastbuf = "Packet_sent:275";
	tcp_sent(pcb, client_sent);
...
void lwip_device_init(int count)
{
	...
	while(1)
	{
		...
		if((flags.first_packet == 1) && (!flags.data_posting_complete) && (!flags.stop_acq))
		{
			DMA_INIT_PROCESS();
			flags.data_posting_complete = true;
			// send_packet();
		...
		if((count_callback == 1) && (flags.data_posting_complete == 1) && (!flags.stop_acq))
		{
			send_packet();
			count_callback = 0;
			flags.last_packet = true;
		...
		if((flags.last_packet == 1) && (!flags.stop_acq))
		{
			//send_lastpacket();
			//flags.last_packet = false;
			packet_sent_complete = 1;
		...
		if((flags.data_recvd == 1) && (!flags.stop_acq))
		{
			flags.data_recvd = false;
		...
		if((flags.first_packet == 1) && (flags.data_posting_complete == 1) && (flags.last_packet == 1) && (!flags.stop_acq))
		{
		...
		if((flags.data_recvd == 1) && (flags.stop_acq) && (flags.start_acq == 1))
		{
...
void lwip_device_init_menu(void)
{
	...
	while(1)
	{
		...
		if((flags.first_packet == 1) && (flags.data_posting_complete == 1) && (flags.last_packet == 1))
		{
			sleep(data_value.acquisition_interval);
